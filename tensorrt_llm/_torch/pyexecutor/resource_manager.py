...
class KVCacheManagerV2(BaseResourceManager):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert quota != float(
            'inf'
        ), "Quota is not set. Please check the kv_cache_config.max_tokens or kv_cache_config.max_gpu_total_bytes"
        logger.info(
            f"KV cache manager v2 device quota is set to {quota / (1 << 30)}GiB"
        )
        ...
        if kv_cache_config.host_cache_size is not None and kv_cache_config.host_cache_size > 0:
            ...
            logger.info(
                f"KV cache manager v2 host cache quota is set to {kv_cache_config.host_cache_size / (1 << 30)}GiB"
            )
